...
namespace Ego.PDF.Printf
{
    public static class SprintfTools
    {
        ...
        {
            Console.Write(SprintfTools.sprintf(Format, Parameters));
        }
        #endregion
        #region fprintf
        public static void fprintf(TextWriter Destination, string Format, params object[] Parameters)
        {
            Destination.Write(SprintfTools.sprintf(Format, Parameters));
        }
        #endregion
        #region sprintf
                                            ...
                                            char Padding, object Value)
        {
            ...
            if (IsNumericType(Value))
            {
                ...
                if (Left2Right || Padding == ' ')
                {
                    if (Alternate && w != "0")
                        w = "0" + w;
                    w = String.Format(System.Globalization.CultureInfo.InvariantCulture, lengthFormat, w);
                }
                else
                {
                    if (FieldLength != int.MinValue)
                                            ...
                                            char Padding, object Value)
        {
            ...
            if (IsNumericType(Value))
            {
                w = String.Format(System.Globalization.CultureInfo.InvariantCulture, numberFormat, Value);
